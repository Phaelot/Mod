...
namespace SovereigntyTK.Game.Battle
{
	public class TacticalBattleController
	{
		...
		private void CheckForMilitaryVictory()
		{
			...
			int num2 = this.Defender.Units.Count((WorkingUnit x) => !x.Disabled);
			if (num == 0)
			{
				this.DeclareWinner(this.Defender);
				return;
		...
		private void GrantPlunder(WorkingUnit Unit)
		{
			...
			int plunder = (int)((float)this.Node.Province.CurrentLoot / num);
			if (this.OnPlunderModRequest != null)
			{
				this.OnPlunderModRequest(this, Unit, ref plunder);
			}
			this.AttackerRealm.Gold.Value += plunder;
			...
			if (plunder > 0)
			{
				GameText plunderLog = GameText.CreateFromLiteral(this.AttackerRealm.Name + " plunders " + plunder + " gold from " + this.Node.Province.Name + ".");
				this.Game.GameCore.FireEvent("BattleLogEvent", new object[] { plunderLog });
			}
			if (Unit != null && !Unit.Disabled && Unit.BattleData != null)
			{
				GameText gameText = GameText.CreateLocalised("EFFECT_PLUNDER", new object[] { plunder });
