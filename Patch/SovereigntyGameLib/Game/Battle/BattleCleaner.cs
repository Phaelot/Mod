...
namespace SovereigntyTK.Game.Battle
{
	public class BattleCleaner
	{
		...
		private void CleanupTacticalBattle(TacticalBattleController Battle)
		{
			...
			this.AwardHeroBattleWin(battleCleanupData);
			...
			this.DamageProvinceFortificationsAfterAssault(battleCleanupData);
		...
		private void DamageProvinceFortificationsAfterAssault(BattleCleanupData Data)
		{
			if (Data == null || Data.Node == null || Data.Node.Province == null)
			{
				return;
			}
			...
			WorkingProvince province = Data.Node.Province;
			int fortLevel = (int)province.FortLevel;
			if (fortLevel <= 0)
			{
				return;
			}
			int fortsToDestroy = Math.Max(1, fortLevel / 2);
			for (int i = 0; i < fortsToDestroy; i++)
			{
				if ((int)province.FortLevel <= 0)
				{
					break;
				}
				province.RemoveFort();
 		...
 		private void AwardHeroBattleWin(BattleCleanupData Data)
 		{
 			if (Data == null || Data.Winner == null || Data.Winner.Hero == null)
 			{
 				return;
 			}
 			Data.Winner.Hero.BattlesWon++;
 		}
