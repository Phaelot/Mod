...
namespace SovereigntyTK.Game
{
	public class SovereigntyGame
	{
		...
		private readonly string[] HumanHeroNames = new string[]
		{
			"Alaric", "Soren", "Edric", "Cedric", "Aldren", "Rowena", "Isolde", "Mira", "Aveline", "Gareth", "Darian", "Serena", "Lucan", "Marwen", "Theodric", "Elena"
		};

		private readonly string[] ElfHeroNames = new string[]
		{
			"Eldara", "Caelir", "Maerwen", "Thalion", "Aelwen", "Lethariel", "Faelar", "Sylvara", "Eryndor", "Naelith", "Ithilwen", "Vaelion", "Araleth", "Celemir", "Lorien", "Nimriel"
		};

		private readonly string[] DwarfHeroNames = new string[]
		{
			"Brokk", "Durgan", "Borin", "Thrain", "Kargan", "Runa", "Hilda Stonehand", "Odrin", "Bramli", "Grimnar", "Dagna", "Torvik", "Kelda", "Baldrek", "Frerin", "Morna"
		};

		private readonly string[] OrcHeroNames = new string[]
		{
			"Gorbad", "Mogra", "Urzak", "Grash", "Vorga", "Thrak", "Muzgob", "Brakka", "Drogul", "Skarn", "Morzag", "Ghorza", "Rukhar", "Zogran", "Bagrat", "Ulgash"
		};

		private readonly string[] UndeadHeroNames = new string[]
		{
			"Vorath", "Mordain", "Ashkarn", "Severin", "Nerath", "Vhalor", "Morvayne", "Kharon", "Zarath", "Maldris", "Ossian", "Velkhar", "Nythrax", "Dread Veyr", "Sable Mor", "Azrul"
		};

		private readonly string[] OutcastHeroNames = new string[]
		{
			"Kestrel", "Rook", "Veyra", "Marek", "Sable", "Corvin", "Nyra", "Ash", "Varek", "Mira Grey", "Dain", "Lysa", "Riven", "Kael", "Tamsin", "Orlan"
		};

		private readonly string[] MonsterHeroNames = new string[]
		{
			"Grum", "Skitter", "Maw", "Bileclaw", "Ghor", "Narg", "Vesh", "Rattlehide", "Bloodmaw", "Cragtooth", "Snarl", "Bogrot", "Murk", "Grizzle", "Fangor", "Thornback"
		};

		private readonly string[] GiantHeroNames = new string[]
		{
			"Bront", "Hargan", "Skeld", "Ulfar", "Gromund", "Torag", "Kelda", "Morna", "Rurik", "Hroth", "Sten", "Borga", "Grunvald", "Ymir", "Thora", "Hulda"
		};

		private readonly string[] DragonHeroNames = new string[]
		{
			"Azharion", "Varkhazar", "Syrath", "Kaelthar", "Morvyr", "Zalthuun", "Aurelion", "Vyrmion", "Nythrakar", "Orryx", "Valkaur", "Drazhul", "Pyraxis", "Thalaz", "Xerath", "Kharzun"
		};
		...
		public void CheckWarStatus()
		{
			...
			string path = System.IO.Path.Combine(System.IO.Path.GetTempPath(), "sovwars.txt");
		...
		public void Dispose()
		{
			...
			GameCore.UnregisterEvent(HandleTickerMessage, "TickerMessage");
		...
		internal void StartPlayerTurn(WorkingRealm Realm)
		{
			...
			StackAnomalyLogger.CheckAllStacks(this, "BeforeAttrition", Realm);
			EconomyController.ApplyAttrition(Realm);
		...
		internal bool MoveUnit(WorkingStack OldStack, WorkingStack NewStack, WorkingUnit Unit, SovereigntyTK.Game.Path MovePath, bool IgnorePackChecks = false)
		{
			...
			if (NewStack.Units.Count >= 20 && !IgnorePackChecks)
			{
				return false;
			}
		...
		internal WorkingHero CreateHero(WorkingRealm Realm, HeroClassData HeroType)
		{
			...
			workingHero.DisplayName = this.CreateLiteralTextKey(this.GetRandomHeroPersonalName(Realm, HeroType));
			AllHeroes.Add(workingHero.ID, workingHero);
			return workingHero;
		}

		private string CreateLiteralTextKey(string Text)
		{
			return "LITERAL:" + Text;
		}

		private string GetRandomHeroPersonalName(WorkingRealm Realm, HeroClassData HeroType)
		{
			string[] names = this.GetHeroNamePool(Realm);
			string name = names[this.RNG.Next(names.Length)];
			string className = "";
			if (HeroType != null && !string.IsNullOrEmpty(HeroType.ClassName))
			{
				className = HeroType.ClassName;
				string[] titledClasses = new string[]
				{
					"Warlord",
					"Mercenary Captain",
					"Taskmaster",
				...
				if (titledClasses.Contains(className))
				{
					switch (Realm.Race)
					{
					case Races.Human:
			...
			{
				name = name + " the " + className;
			}
			return name;
		}

		private string[] GetHeroNamePool(WorkingRealm Realm)
		{
			if (Realm == null)
			{
				return this.HumanHeroNames;
			}
			switch (Realm.Race)
			{
			case Races.Human:
				return this.HumanHeroNames;
			case Races.Elf:
				return this.ElfHeroNames;
			case Races.Dwarf:
				return this.DwarfHeroNames;
			case Races.Orc:
				return this.OrcHeroNames;
			case Races.Undead:
				return this.UndeadHeroNames;
			case Races.Outcast:
				return this.OutcastHeroNames;
			case Races.Monster:
				return this.MonsterHeroNames;
			case Races.Giant:
				return this.GiantHeroNames;
			case Races.Dragon:
				return this.DragonHeroNames;
			default:
				return this.HumanHeroNames;
			}
		}
		...
		public void ChangeProvinceOwner(WorkingProvince Province, WorkingRealm Realm)
		{
			...
			if (Province.IsCapitol)
			{
				...
				foreach (WorkingStack item3 in ownerRealm.Stacks.ToList())
				{
					...
					else if (item3.Node.Province != null && item3.Node.Province.OwnerRealm == RebelRealm)
					{
						if (item3.Hero != null)
						{
				...
				foreach (UnitQueueItem queueItem in ownerRealm.GetCurrentUnitQueue().ToList())
				{
					if (queueItem.Unit != null)
					{
						DestroyUnit(queueItem.Unit);
					}
				}
				foreach (WorkingRealm value in AllRealms.Values)
				{
		...
		internal void StartGameTurn()
		{
			...
			Marketplace.Update();
			RealmTurnStatusLogger.WriteTurnStatus(this);
			Autosave();
