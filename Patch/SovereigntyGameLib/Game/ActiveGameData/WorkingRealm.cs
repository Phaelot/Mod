...
namespace SovereigntyTK.Game.ActiveGameData
{
	public class WorkingRealm
	{
		...
		private bool ProvinceAllowsCampaignRecovery(WorkingProvince Province)
		{
			if (Province == null)
			{
				return true;
			}
			if (Province.Occupied && Province.OccupierRealm != null && Province.OccupierRealm != this && this.DiplomacyManager.GetRelation(Province.OccupierRealm) != RelationStates.Alliance)
			{
				return false;
			}
			if (Province.OwnerID == this.ID)
			{
				return true;
			}
			if (Province.OwnerRealm != null && this.DiplomacyManager.GetRelation(Province.OwnerRealm) == RelationStates.Alliance)
			{
				return true;
			}
			return false;
		...
		internal void ResetUnitMoves()
		{
			foreach (WorkingUnit workingUnit in this.Units)
			{
				...
				if (!workingUnit.HasBattled)
				{
					...
					{
						WorkingProvince prov = workingUnit.OwnerStack.Node.Province;
						inEnemyTerritory = !this.ProvinceAllowsCampaignRecovery(prov);
						if (!inEnemyTerritory && this.Game.EconomyController.RecentlyConqueredProvinces != null && this.Game.EconomyController.RecentlyConqueredProvinces.Contains(prov.ID))
						{
