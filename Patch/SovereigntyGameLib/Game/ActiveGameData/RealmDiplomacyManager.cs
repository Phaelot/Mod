...
namespace SovereigntyTK.Game.ActiveGameData
{
	public class RealmDiplomacyManager
	{
		...
		public RelationStates GetRelation(WorkingRealm TargetRealm)
		{
			...
			// During save loading, buildings and spells can be reconstructed before
			// SovereigntyGame.RebelRealm is assigned.  Several scripts query
			// WorkingRealm.Enemies while loading; the old code dereferenced
			// Game.RebelRealm.Name here and crashed.  Treat the special rebel
			// relation only after RebelRealm exists.
			WorkingRealm rebelRealm = (this.Game != null) ? this.Game.RebelRealm : null;
			if (rebelRealm != null)
			{
				if (this.Realm == rebelRealm.Name)
				{
					return RelationStates.War;
				}
				if (TargetRealm == rebelRealm)
				{
					return RelationStates.War;
				}
			}
