		...
		private void ExecuteCommand(ConsoleCommand Command)
		{
			switch (Command.CommandName)
			{
				...
				case "/fogofwar":
					...
					else if (Command.Parameters.Count < 1 || Command.Parameters.Count > 1)
					{
						AddOutputLine("Expected parameters: [On|Off]");
					}
					else if (Game.Map == null)
					{
						AddOutputLine("Map not available");
					}
					else if (Command.Parameters[0] == "On")
					{
						Game.Map.SimpleFogOfWarEnabled = true;
						Game.Map.RefreshMode();
						AddOutputLine("Fog of war enabled");
					}
					else if (Command.Parameters[0] == "Off")
					{
						Game.Map.SimpleFogOfWarEnabled = false;
						Game.Map.RefreshMode();
						AddOutputLine("Fog of war disabled. All army stacks should now be visible and inspectable.");
					}
					else
					{
						AddOutputLine("Expected parameters: [On|Off]");
