...
namespace SovereigntyTK.UI.Map
{
	public class CampaignMap
	{
		...
		public bool SimpleFogOfWarEnabled = true;
		...
		private bool RealmProvidesVision(WorkingRealm realm)
		{
			if (Game == null || Game.CurrentGame == null || Game.CurrentGame.PlayerRealm == null || realm == null)
			{
				return false;
			}
			if (realm == Game.CurrentGame.PlayerRealm)
			{
				return true;
			}
			return realm.DiplomacyManager.GetRelation(Game.CurrentGame.PlayerRealm) == RelationStates.Alliance;
		}

		private bool ProvinceProvidesVision(WorkingProvince province)
		{
			return province != null && RealmProvidesVision(province.OwnerRealm);
		}
		...
		private bool StackProvidesSimpleScoutVision(WorkingStack stack)
		{
			if (stack == null || stack.Owner != Game.CurrentGame.PlayerRealm || stack.Node == null)
			{
				return false;
			}
			return stack.Units.Any((WorkingUnit x) => UnitProvidesScoutLikeVision(x));
		}

		private void AddRegionAndNeighbours(HashSet<string> visibleRegions, GameRegion region)
		{
			...
			foreach (GameRegion allConnectedRegion in region.GetAllConnectedRegions())
			{
				if (allConnectedRegion != null && !string.IsNullOrEmpty(allConnectedRegion.Name))
				{
					visibleRegions.Add(allConnectedRegion.Name);
				}
			}
		}

		public HashSet<string> GetSimpleVisibleRegions()
		{
			HashSet<string> hashSet = new HashSet<string>();
			if (Game == null || Game.CurrentGame == null || Game.CurrentGame.PlayerRealm == null)
			{
				return hashSet;
			}
			foreach (WorkingProvince value in Game.CurrentGame.AllProvinces.Values)
			{
				if (!ProvinceProvidesVision(value))
				{
					continue;
				}
				AddRegionAndNeighbours(hashSet, value);
			}
			foreach (WorkingStack value2 in Game.CurrentGame.AllStacks.Values)
			{
				if (StackProvidesSimpleScoutVision(value2))
				{
					AddRegionAndNeighbours(hashSet, value2.Node.GetRegion());
				}
			}
			return hashSet;
		}

		public bool IsRegionVisible(GameRegion region)
		{
			if (region == null)
			{
				return false;
			}
			if (Game == null || Game.CurrentGame == null)
			{
				return true;
			}
			if (!SimpleFogOfWarEnabled)
			{
				return true;
			}
			return GetSimpleVisibleRegions().Contains(region.Name);
		}

		private bool ShouldDrawSimpleFogOverlay()
		{
			return SimpleFogOfWarEnabled && CurrentMode == MapModes.Default;
		}

		private void ApplySimpleFogOfWar()
		{
			if (Game == null || Game.CurrentGame == null || Game.CurrentGame.PlayerRealm == null)
			{
				return;
			}
			if (CurrentMode == MapModes.Default)
			{
				foreach (WorkingProvince value in Game.CurrentGame.AllProvinces.Values)
				{
					Renderer.SetProvinceColour(value.Name, Color.Transparent);
				}
				foreach (WorkingZone value2 in Game.CurrentGame.AllZones.Values)
				{
					Renderer.SetZoneColour(value2.Name, Color.Transparent);
				}
			}
			if (ShouldDrawSimpleFogOverlay())
			{
				HashSet<string> simpleVisibleRegions = GetSimpleVisibleRegions();
				Color color = Color.FromArgb(110, 90, 90, 90);
				foreach (WorkingProvince value3 in Game.CurrentGame.AllProvinces.Values)
				{
					if (!simpleVisibleRegions.Contains(value3.Name))
					{
						Renderer.SetProvinceColour(value3.Name, color);
					}
				}
				foreach (WorkingZone value4 in Game.CurrentGame.AllZones.Values)
				{
					if (!simpleVisibleRegions.Contains(value4.Name))
					{
						Renderer.SetZoneColour(value4.Name, color);
					}
				}
			}
			foreach (WorkingProvince value5 in Game.CurrentGame.AllProvinces.Values)
			{
				value5.UpdateMapVisibilityState();
			}
			foreach (WorkingStack value6 in Game.CurrentGame.AllStacks.Values)
			{
				value6.UpdateSprite();
			}
		}
		...
		internal void SetNearestNode(ActivePathNode Node)
		{
			...
			else
			{
				...
				if (path.DoesCrossRiver())
				{
					Renderer.SetArrowColour(Color.FromArgb(255, 50, 100, 180));
				}
				else
				{
					Renderer.SetArrowColour(Color.FromArgb(255, 47, 65, 38));
				}
				Sovereignty game3 = Game;
				if (NearestNode.Province != null && NearestNode.Province.OwnerID != Game.CurrentGame.PlayerRealm.ID)
				{
					string attritionTerrain = "";
					float attritionMult = 1f;
					try
					{
						if (NearestNode.GetRegion() != null && NearestNode.GetRegion().Terrain != null)
						{
							attritionTerrain = NearestNode.GetRegion().Terrain.BaseType.ToLowerInvariant();
						}
					}
					catch
					{
					}
					if (attritionTerrain == "swamp" || attritionTerrain == "wasteland") attritionMult = 2f;
					else if (attritionTerrain == "old forest" || attritionTerrain == "mountain") attritionMult = 1.5f;
					float stackMult = System.Math.Max(0.5f, (float)ActiveStack.Units.Count / 10f);
					float estAttrition = 2f * stackMult * attritionMult;
					if (ActiveStack.Hero != null)
					{
						estAttrition *= 0.7f;
					}
					int estDamage = System.Math.Max(1, (int)System.Math.Ceiling(estAttrition));
					GameText attritionText = GameText.CreateFromLiteral("⚠ Attrition: ~" + estDamage + " HP/turn per unit" + (attritionMult > 1f ? " (harsh terrain)" : "") + (ActiveStack.Hero != null ? " (hero reduces)" : ""));
					game3.FireEvent("TooltipChanged", new object[] { attritionText });
				}
				else
				{
		...
		internal void HandleProvinceClicked(string ProvinceName, MouseButton Button)
		{
			...
			if ((CurrentMode == MapModes.Political || CurrentMode == MapModes.Economy || CurrentMode == MapModes.Plague || CurrentMode == MapModes.Relations) && Button == MouseButton.Right)
			{
		...
		public void RefreshMode()
		{
			switch (CurrentMode)
			{
				...
				case MapModes.Plague:
					{
						Color healthyColour = Color.FromArgb(60, 0, 0, 0);
						Color susceptibleColour = Color.FromArgb(120, 90, 80, 0);
						Color immuneColour = Color.FromArgb(190, 120, 180, 230);
						Color plagueColour = Color.FromArgb(230, 0, 0, 0);
						foreach (WorkingProvince province in Game.CurrentGame.AllProvinces.Values)
						{
							Color provinceColour = healthyColour;
							if (province.PlagueTurns > 0)
							{
								provinceColour = plagueColour;
							}
							else if (province.PlagueImmuneTurns > 0)
							{
								provinceColour = immuneColour;
							}
							else if (province.PlaguePossible())
							{
								provinceColour = susceptibleColour;
							}
							Renderer.SetProvinceColour(province.Name, provinceColour);
						}
						foreach (WorkingZone zone in Game.CurrentGame.AllZones.Values)
						{
							Renderer.SetZoneColour(zone.Name, Color.FromArgb(120, 0, 0, 0));
						}
						break;
					}
				case MapModes.StackMove:
		...
		internal void ShowAttackArrow(Path AttackPath)
		{
			if (AttackPath != null)
			{
				...
				if (AttackPath.DoesCrossRiver())
				{
					Renderer.SetArrowColour(Color.FromArgb(255, 180, 80, 30));
				}
				else
				{
