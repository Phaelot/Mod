...
namespace SovereigntyTK.AI.V2
{
	public class AIWarManager
	{
		...
		private int GetTrait(string TraitName)
		{
			AITraitData aITraitData;
			if (AI == null || AI.Game == null || AI.Game.Data == null || AI.Game.Data.AITraits == null || AI.Realm == null || !AI.Game.Data.AITraits.TryGetValue(AI.Realm.Name, out aITraitData))
			{
				return 0;
			}
			switch (TraitName)
			{
			case "Warmonger":
				return aITraitData.Warmonger;
			case "Militarist":
				return aITraitData.Militarist;
			case "Opportunist":
				return aITraitData.Opportunist;
			case "Diplomat":
				return aITraitData.Diplomat;
			default:
				return 0;
			}
		}

		private int GetWarCooldown()
		{
			int warmonger = GetTrait("Warmonger");
			if (warmonger >= 10)
			{
				return AI.RNG.Next(2);
			}
			if (warmonger >= 8)
			{
				return AI.RNG.Next(2) + 1;
			}
			return AI.RNG.Next(3) + 2;
		}

		private int GetWarReasonThreshold(WorkingRealm TargetRealm)
		{
			int threshold = 10;
			threshold -= GetTrait("Warmonger") / 2;
			threshold -= GetTrait("Opportunist") / 4;
			if (AI.Realm.Provinces.Count < 8)
			{
				threshold -= 2;
			}
		...
		public bool DefeatIsLikely(WorkingRealm Realm, bool UseCachedValues)
		{
			...
			num3 /= 3f;
			num = num3 / num2;
			num -= (float)GetTrait("Warmonger") * 0.03f;
			num -= (float)GetTrait("Militarist") * 0.01f;
		...
		public bool VictoryIsPossible(WorkingRealm Realm)
		{
			...
			num += (float)GetTrait("Warmonger") * 0.02f;
			num += (float)GetTrait("Militarist") * 0.01f;
			num -= (float)AI.Realm.Enemies.Count * 0.2f;
		...
		public int GetPeaceDesire(WorkingRealm Enemy)
		{
			...
			num -= (float)GetTrait("Warmonger") * 1.5f;
			num -= (float)GetTrait("Militarist") * 0.5f;
			num += (float)GetTrait("Diplomat") * 0.5f;
			if (num > 100f)
			{
				num = 100f;
			}
		...
		public float GetWarValue(WorkingRealm Realm)
		{
			...
			num += (float)GetTrait("Warmonger") * 2f;
			num += (float)GetTrait("Militarist") * 0.5f;
			num += (float)GetTrait("Opportunist") * 0.5f;
			num -= (float)GetTrait("Diplomat") * 1.5f;
		...
		private float GetRealmMilitaryStrength(WorkingRealm Realm, bool UseCachedValues)
		{
			if (Realm == null)
			{
				return 1f;
			}
			if (UseCachedValues && RealmStrengthValues != null && RealmStrengthValues.ContainsKey(Realm))
			{
				return Math.Max(1f, RealmStrengthValues[Realm]);
			}
			return Math.Max(1f, Realm.Units.Where((WorkingUnit x) => x.OwnerStackID > 0 || x.Class == UnitClasses.Fort).Sum((WorkingUnit x) => x.GetValue()));
		}

		private float GetAllianceAdjustedMilitaryStrength(WorkingRealm Realm, bool UseCachedValues)
		{
			float num = GetRealmMilitaryStrength(Realm, UseCachedValues);
			foreach (WorkingRealm ally in Realm.Allies)
			{
				num += GetRealmMilitaryStrength(ally, UseCachedValues) / 4f;
			}
			return Math.Max(1f, num);
		}

		private float GetRealmGrossIncome(WorkingRealm Realm)
		{
			if (Realm == null)
			{
				return 1f;
			}
			return Math.Max(1f, (float)AI.Game.EconomyController.GetRealmTotalIncome(Realm));
		}

		private bool OffensiveWarRiskAcceptable(WorkingRealm TargetRealm, WarReasonData ReasonData, WarGoalData GoalData)
		{
			if (TargetRealm == null || TargetRealm == AI.Game.RebelRealm)
			{
				return true;
			}
			float ourStrength = GetAllianceAdjustedMilitaryStrength(AI.Realm, UseCachedValues: true);
			float targetStrength = GetAllianceAdjustedMilitaryStrength(TargetRealm, UseCachedValues: true);
			float strengthRatio = ourStrength / targetStrength;
			float ourIncome = GetRealmGrossIncome(AI.Realm);
			float targetIncome = GetRealmGrossIncome(TargetRealm);
			float incomeRatio = ourIncome / targetIncome;
			int warmonger = GetTrait("Warmonger");
			int militarist = GetTrait("Militarist");
			int opportunist = GetTrait("Opportunist");
			float requiredStrength = 0.85f - (float)warmonger * 0.01f - (float)militarist * 0.005f;
			float requiredIncome = 0.65f - (float)warmonger * 0.01f - (float)opportunist * 0.005f;
			int distractedWars = TargetRealm.Enemies.Count((WorkingRealm x) => x != AI.Realm);
			float distraction = Math.Min(0.2f, (float)distractedWars * 0.08f);
			requiredStrength -= distraction;
			requiredIncome -= distraction;
			int damagedProvinces = TargetRealm.Provinces.Count((WorkingProvince x) => x.GetUnmodifiedEconomy() == 0 || x.Occupied);
			if (damagedProvinces > 0)
			{
				float damageModifier = Math.Min(0.15f, (float)damagedProvinces * 0.03f);
				requiredStrength -= damageModifier;
				requiredIncome -= damageModifier;
			}
			if (ReasonData != null && ReasonData.ReasonType == WarReasons.LandReclaim)
			{
				requiredStrength -= 0.05f;
				requiredIncome -= 0.05f;
			}
			if (GoalData != null && GoalData.GoalType == WarGoalTypes.LootRealm)
			{
				requiredStrength -= 0.05f;
				requiredIncome -= 0.05f;
			}
			if (requiredStrength < 0.55f)
			{
				requiredStrength = 0.55f;
			}
			if (requiredIncome < 0.4f)
			{
				requiredIncome = 0.4f;
			}
			float hardMinimumStrength = 0.45f - distraction;
			if (hardMinimumStrength < 0.3f)
			{
				hardMinimumStrength = 0.3f;
			}
			AI.Log("      War risk: strength " + strengthRatio.ToString("0.00") + " / " + requiredStrength.ToString("0.00") + ", income " + incomeRatio.ToString("0.00") + " / " + requiredIncome.ToString("0.00") + ", target distracted wars " + distractedWars);
			if (strengthRatio < hardMinimumStrength)
			{
				AI.Log("      Rejecting war goal: enemy military superiority too high");
				return false;
			}
			if (strengthRatio < requiredStrength && incomeRatio < requiredIncome)
			{
				AI.Log("      Rejecting war goal: target is too strong economically and militarily");
				return false;
			}
			return true;
		}
		...
		private bool HasSharedBorder(WorkingRealm Realm)
		{
			foreach (WorkingProvince province in AI.Realm.Provinces)
			{
				foreach (GameRegion allConnectedRegion in province.GetAllConnectedRegions())
				{
					...
					{
						if (allConnectedRegion2 is WorkingProvince && (allConnectedRegion2 as WorkingProvince).OwnerRealm == Realm)
						{
							return true;
						}
					}
		...
		internal void DeclareWars()
		{
			...
			source = source.OrderByDescending((WorkingRealm x) => GetWarValue(x)).ToList();
			...
			foreach (WorkingRealm item in source)
			{
				...
				foreach (WarReasonData item3 in list)
				{
					...
					{
						if (!OffensiveWarRiskAcceptable(item, item3, item4))
						{
							continue;
						}
		...
		{
			int num = GetWarReasonThreshold(TargetRealm);
			AI.Log("    " + Reasons + " value " + (int)Value + " threshold " + num);
		...
		private int GetExpansionDesire()
		{
			...
			num += GetTrait("Warmonger") * 2;
			num += GetTrait("Militarist");
			num += GetTrait("Opportunist") / 2;
