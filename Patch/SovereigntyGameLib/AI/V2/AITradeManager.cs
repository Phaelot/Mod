...
namespace SovereigntyTK.AI.V2
{
	public class AITradeManager
	{
		...
		private void LogTrade(string Text)
		{
			try
			{
				this.AI.Log(Text);
			...
			this.AppendTradeLogLine(Text);
		}

		private void AppendTradeLogLine(string Text)
		{
			try
			{
				...
				string filePath = System.IO.Path.Combine(logDir, "AITradeManager.log");
		...
		internal bool AttemptTrade(TradeOfferList Demands, WorkingRealm TargetRealm, bool Immediate = false)
		{
			...
			{
				string text;
				if (!RealmTradeManager.CanStartEconomicTrade(this.AI.Realm, TargetRealm, out text))
				{
					this.LogTrade("    Trade failed, " + text);
					this.LogTrade("[TradeRule] Blocked AI trade " + this.AI.Realm.Name + " -> " + TargetRealm.Name + ": " + text);
					return false;
				}
			}
			if (this.AI.Realm.GetTradeAgent() == null)
			{
				this.LogTrade("    Trade failed, no agent available");
				return false;
			}
			if (this.TradeOfferCooldowns.ContainsKey(TargetRealm))
			{
				this.LogTrade("    Trade failed, traded with " + TargetRealm.Name + " too recently");
				return false;
			}
			if (this.AI.Game.IgnoreHumanPlayer && TargetRealm == this.AI.Game.PlayerRealm)
			{
				this.LogTrade("    Trade failed, human realm is being ignored");
				return false;
			}
			...
			{
				this.LogTrade("    Checking for prisoners...");
				...
				{
					this.LogTrade("    Offering " + realmPrisoners.Count + " prisoners");
				...
				{
					this.LogTrade("    Demanding " + realmPrisoners2.Count + " prisoners");
				...
				{
					this.LogTrade("    No prisoners to be exchanged");
				}
			}
			float num = this.GetRealmTradeValue(TargetRealm, tradeOffer);
			this.LogTrade("    Trade value calculated at " + num);
			...
			if (flag)
			{
				this.LogTrade("    Resources wil be used before gold");
			}
			else
			{
				this.LogTrade("    Gold will be used before resources");
			}
			while (num > 0f && !Immediate)
			{
				if (availablePrisoners.Count > 0 && this.AI.Realm.DiplomacyManager.GetDisposition(TargetRealm) > 0f)
				{
					...
					this.LogTrade("    Added prisoner, value is now " + num);
				}
				else if (flag)
				{
					if (availableResources.Count > 0)
					{
						...
						num = this.GetRealmTradeValue(TargetRealm, tradeOffer);
						this.LogTrade(string.Concat(new object[] { "    Added ", num3, " ", resourceData.ResourceName, ", value is now ", num }));
					}
					else
					{
						...
						this.LogTrade(string.Concat(new object[]
						{
							"    Added ",
				...
				else
				{
					...
					num = this.GetRealmTradeValue(TargetRealm, tradeOffer);
					this.LogTrade(string.Concat(new object[] { "    Added ", num5, " ", resourceData2.ResourceName, ", value is now ", num }));
				}
			}
			if (num < 0f || Immediate)
			{
				this.TradeOfferCooldowns.Add(TargetRealm, 3 + this.AI.RNG.Next(3));
				this.LogTrade("    Trade is balanced, offer sent to " + TargetRealm.Name);
				if (Immediate)
				{
			...
			this.LogTrade("    Unable to balance trade, aborting");
			return false;
		}
		...
		public AITradeResponses EvaluateTradeOffer(WorkingRealm OfferingRealm, TradeOffer Offer)
		{
			...
			{
				string text;
				if (!RealmTradeManager.CanStartEconomicTrade(OfferingRealm, this.AI.Realm, out text))
				{
					this.LogTrade("[TradeRule] Rejected trade offer " + OfferingRealm.Name + " -> " + this.AI.Realm.Name + ": " + text);
