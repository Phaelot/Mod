...
namespace SovereigntyTK.Game.ActiveGameData
{
	public class WorkingStack
	{
		...
		public void TransferFromStack(WorkingUnit Unit, Path MovePath, bool IgnorePackChecks = false)
		{
			...
			List<Point> list;
			if (MovePath.PathPoints.Count > 0)
			{
			...
			{
				list = new List<Point>();
				list.Add(Unit.OwnerStack.Node.MapCoords);
		...
		public bool ScoutActive()
		{
			...
			if (this.Node.Province != null && this.Node.Province.OwnerRealm.DiplomacyManager.GetRelation(this.Game.PlayerRealm) == RelationStates.Alliance)
			{
				return true;
			}
			...
			if (this.Game.PlayerRealm.HasStatus("SpecialScout", new object[] { this.Node.Province }))
			{
				return true;
			}
			List<WorkingProvince> list = null;
			...
			foreach (WorkingProvince workingProvince in list)
			{
				if (workingProvince != this.Node.Province && workingProvince.LandNode.CurrentStack != null && workingProvince.LandNode.CurrentStack.Owner == this.Game.PlayerRealm)
				{
					if (workingProvince.LandNode.CurrentStack.Units.Count((WorkingUnit x) => x.HasStatus("Scout", new object[0])) > 0)
					{
						return true;
					}
				}
