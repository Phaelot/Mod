	...
	public class BattleMap
	{
		...
		private bool UnitMoveTargetTileValid(Point Tile)
		{
			...
			if (MovingUnit.BattleData.BattleLocation != Tile)
			{
				...
				else if (MovingUnit.MoveType == MoveTypes.Air)
				{
					if (GetAdjacentEnemies(Tile, MovingUnit.OwnerRealmID).Count((WorkingUnit x) => x.MoveType == MoveTypes.Air) > 0)
					{
						return false;
					}
				}
				else if (GetAdjacentEnemies(Tile, MovingUnit.OwnerRealmID).Count((WorkingUnit x) => x.MoveType != MoveTypes.Phantom) > 0)
				{
					return false;
				}
			}
